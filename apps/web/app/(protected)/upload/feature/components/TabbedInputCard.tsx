...
export function TabbedInputCard({
  ...
  const tabClassName = cn(
    ...
    "gap-1 px-1 py-2 text-xs sm:gap-2 sm:px-3 sm:text-sm",
  ...
  const tabHeightClass = 
    tab === "import" ? "h-[460px] sm:h-[520px]" : "h-[300px] sm:h-[320px]";

  return (
    <div className={className}>
      <Tabs value={tab} onValueChange={(v) => setTab(v as TabKey)} className="gap-0">
        <div className="overflow-hidden rounded-lg border border-foreground/20 shadow-sm sm:rounded-xl">

          {/* -------------------------
          * Top Bar (input type tabs)
          * ------------------------- */}
          <div className="font-ui h-12 bg-card px-1 sm:px-2 border-b">
            <TabsList className="grid h-full w-full grid-cols-3 items-stretch gap-1 bg-transparent p-0 sm:gap-4">
              {/* -------------------------
              * Gutenberg Ebook Import
              * ------------------------- */}
              <TabsTrigger value="import" className={tabClassName}>
                <BookSearch className="h-4 w-4" />
                <span className="min-w-0 truncate">{uploadMsgs.tabs.importEbook}</span>
              </TabsTrigger>
              {/* -------------------------
              * File Upload
              * ------------------------- */}
              <TabsTrigger value="upload" className={tabClassName}>
                ...
                <span className="min-w-0 truncate">{uploadMsgs.tabs.uploadFile}</span>
              </TabsTrigger>
              {/* -------------------------
              * Paste
              * ------------------------- */}
              <TabsTrigger value="paste" className={tabClassName}>
                ...
                <span className="min-w-0 truncate">{uploadMsgs.tabs.pasteText}</span>
              </TabsTrigger>
            </TabsList>
          </div>
            ...
            * ------------------------- */}
            <TabsContent value="upload" className="m-0 h-full flex">
              <UploadSurface 
                ...
                className="h-full flex-1 rounded-none bg-muted/20 p-4 font-ui sm:p-6"
              />
            </TabsContent>
            {/* -------------------------
            * ( Paste ): Text input
