...
type targetClickMessages = (typeof messages)[UiLang]["reader"]["helpPopover"]["targetClick"]

export function TargetClickInfo({ msgs }: { msgs: targetClickMessages }) {
  return (
      ...
      ">
        ...
        <div className="flex items-center gap-2 px-3 py-2 text-[12px] text-muted-foreground">
          ...
          <span className="font-bold">
            {msgs.header.prefix}{" "}
            <span className="bg-orange-500/20">{msgs.header.highlight}</span>
            {" "}{msgs.header.suffix}
          </span>
        </div>
        ...
        <div className="flex w-full flex-col gap-1 border-t border-border/70 bg-background/70 px-3 py-2 text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-1">
            ...
            <span>{msgs.defineAndExplain}</span>
          </span>
