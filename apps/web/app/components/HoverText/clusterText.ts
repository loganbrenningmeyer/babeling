...
export function buildClusters(words: string[], spaces: string[]): Cluster[] {
  ...
  const isApostropheClitic = (tok: string | undefined) => {
    if (!tok) return false;
    return /^[’'][A-Za-z]+$/.test(tok); // "’s", "'re", "'ve", etc.
  };
  ...
  while (i < words.length) {
    ...
    // Opening punct clings forward: « word  -> «word
    if (
    ...
    // Trailing punct clings backward: word ,  -> word,
    ...
    // Dot runs after the above loop: word... (tokenized ".", ".", ".")
    while (words[end + 1] === "." && isInlineNoNewlineSpace(spaces[end] ?? "")) {
      text += (spaces[end] ?? "") + ".";
      end += 1;
    }

    // Glue hyphen-prefixed pieces like "-t" "-il" (common in French inversion tokenization)
    while (
    ...
    // Apostrophe clitics as one token: prince + "'s"  -> prince's
    ...
    // Split apostrophe + suffix: prince + "’" + "s"  -> prince’s
    if (
