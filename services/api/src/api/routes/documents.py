    ...
    *,
    ...
    try:
        ...
        doc = Document(
            ...
            source_file_title=None,
            source_file_author=None,
            source_file_language=None,
            cover_image_id=None,
        )
        db.add(doc)
        db.flush()
    ...
    *,
    ...
    try:
        ...
        doc = Document(
            ...
            source_file_title=epub_doc.title,
            source_file_author=epub_doc.author,
            source_file_language=epub_doc.language,
        )
        db.add(doc)
        db.flush()
        ...
        # -------------------------
        sec_row_by_key = {row.section_key: row for row in section_rows}
...
def _save_pdf_document(
    *,
    ...
    """
    Given a parsed .pdf's PdfDocument information, splits into pages
    and saves the Document / DocumentPages to database
    """
    # -------------------------
    # Build PdfPages / Per-page texts + Full text
    # -------------------------
    ...
    try:
        ...
        doc = Document(
            ...
            source_file_title=sanitized_source_file_title,
            source_file_author=sanitized_source_file_author,
            source_file_language=sanitized_source_file_language,
            cover_image_id=None,
        )
        db.add(doc)
        db.flush()
        ...
        # -------------------------
        image_rows: list[DocumentImage] = []
        for image_key, img in pdf_doc.images.items():
            if not img.data:
                continue
            row = DocumentImage(
                document_id=doc.id,
        ...
        if pdf_doc.cover_bytes:
            cover_sha = hashlib.sha256(pdf_doc.cover_bytes).hexdigest()
            ...
            if cover_match is not None:
                ...
                cover_row = DocumentImage(
                    ...
                    image_key=cover_key,
                    href="__cover__",
                    media_type=pdf_doc.cover_media_type,
                    byte_length=len(pdf_doc.cover_bytes),
                    content_sha256=cover_sha,
                    data=pdf_doc.cover_bytes,
                )
                db.add(cover_row)
                db.flush()
                cover_image_db_id = cover_row.id
        ...
        # -------------------------
        sec_row_by_key = {row.section_key: row for row in section_rows}
        sec_meta_by_key = {section.key: section for section in pdf_doc.sections}

        for row in section_rows:
            pk = sec_meta_by_key[row.section_key].parent_key
            if pk and pk in sec_row_by_key:
                row.parent_section_id = sec_row_by_key[pk].id
        db.flush()
        ...
        # Insert DocumentPages (map to DocumentSections)
        # -------------------------
        sec_id_by_key = {row.section_key: row.id for row in section_rows}
